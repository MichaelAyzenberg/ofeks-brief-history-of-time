...
const Progress = () => {
  const { isVisited, isFavorite, visitedCount, favoriteCount, resetProgress } = useProgress();
  ...
  const handleReset = () => {
    if (window.confirm('האם אתה בטוח שרוצה לאפס את כל ההתקדמות?')) {
      resetProgress();
    }
  };
  ...
  return (
    <div className="min-h-screen px-4 pt-8 pb-4 max-w-lg mx-auto">
      ...
      >
        ...
        <p className="text-blue-300/60 text-sm">כמה יקום חקרת עד עכשיו</p>
      </motion.div>
      ...
      >
        ...
        <div>
          <div className="text-lg font-bold text-white">{visitedCount} עולמות</div>
          <div className="text-sm text-blue-300/60">מתוך {total} נחקרו</div>
          <div className="flex gap-3 mt-2 text-xs text-blue-300/50">
            <span>⭐ {favoriteCount} מועדפים</span>
            <span>📖 {remaining.length} נותרו</span>
          </div>
        </div>
      </motion.div>
      ...
      >
        <h2 className="text-sm font-bold text-blue-300/60 mb-3">הישגים</h2>
          ...
          {[
            ...
            <div key={i}
              ...
              style={{
                ...
                borderColor: badge.earned ? '#60a5fa40' : '#1a2040',
      ...
      {favorites.length > 0 && (
        ...
        >
          <h2 className="text-sm font-bold text-blue-300/60 mb-3">⭐ המועדפים שלי</h2>
      ...
      {visited.length > 0 && (
        ...
        >
          <h2 className="text-sm font-bold text-blue-300/60 mb-3">✓ נחקרו</h2>
      ...
      {remaining.length > 0 && (
        ...
        >
          <h2 className="text-sm font-bold text-blue-300/60 mb-3">🔭 לגלות עוד</h2>
          <div className="space-y-1">
